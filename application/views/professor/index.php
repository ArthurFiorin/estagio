		...
		<div class="content bg-light">
			...
			<form method="post">
				<div class="form-group mb-4">
					...
					<input class="form-control bg-primary text-light" placeholder="Digite seu usuário" name="loginTxt" id="loginTxt" maxlength="15" />
				</div>
				<div class="form-group mb-4">
				...
				<div class="row m-0 p-0">
					<button type="button" id="loginBtn" class="btn btn-success btn-block">
						ACESSAR
					</button>
				</div>
			</form>
...
<script type="text/javascript" charset="utf-8">
	...
	$(document).ready(function() {
		$('#loginBtn').on('click', async function(e) {
			...
			const request = await fetch(base_url + 'professor/loginProfessor', config);
			...
			if (response.codigo == 1) {
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						window.location.href = '<?= base_url('Funcoes/insertaluno') ?>';
					}
				});
			} else {
				Swal.fire({
